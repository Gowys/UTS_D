...
if (isset($_POST['csimpan'])) {
    ...
    if ($_GET['hal'] == "edit") {
        ...
        //Data akan diedit
        $edit = mysqli_query($koneksi, "UPDATE ukm set
                                        namaukm = '$_POST[tnamaukm]',
                                        notelp = '$_POST[tnotelp]',
                                        createukm = '$_POST[tcreateukm]'
                                        WHERE id_ukm = '$_GET[id]'");
        if ($edit) { //Jika edit Sukses
            echo "<script>
                    alert('Edit data suksess!');
                    document.location='ukm.php';
                 </script>";
        ...
        $simpan = mysqli_query($koneksi, "INSERT INTO ukm (namaukm, notelp, createukm)
                                          VALUES ('$_POST[tnamaukm]',
                                                 '$_POST[tnotelp]',
                                                 '$_POST[tcreateukm]')");
...
if (isset($_GET['hal'])) {
    ...
    if ($_GET['hal'] == "edit") {
        ...
        $tampil = mysqli_query($koneksi, "SELECT * FROM ukm WHERE id_ukm = '$_GET[id]' ");
        $data = mysqli_fetch_array($tampil);
        if ($data) {

            //Jika data ditemukan, maka data ditampung  ke dalam variabel
            $vnamaukm = $data['namaukm'];
            $vnotelp = $data['notelp'];
            $vcreateukm = $data['createukm'];
        }
    } else if ($_GET['hal'] == "hapus") {

        //Persiapan Hapus data
        $hapus = mysqli_query($koneksi, "DELETE FROM ukm where id_ukm = '$_GET[id]'");
        if ($hapus) {
            echo "<script>
...
if (isset($_POST['bcari'])) {
    $keyword = $_POST['tcari'];
    $q = "SELECT * FROM ukm WHERE namaukm like '%$keyword%'
                or notelp like'%$keyword%' or createukm like'%$keyword%'
                order by id_ukm desc ";
} else {
    $q = "SELECT * from ukm order by id_ukm desc";
}
?>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="staticBackdropLabel">Form input UKM</h1>
                            ...
                            <form action="" method="POST">
                                <div class="modal-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="tnamaukm" value="<?= @$vnamaukm ?>" class="form-control" placeholder="Input nama prodi anda disini!" required>
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="tcreateukm" value="<?= @$vcreateukm ?>" class="form-control" placeholder="" required>
                                    </div>
                ...
                <table class="table table-bordered">
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?= $data['namaukm'] ?></td>
                            <td><?= $data['notelp'] ?></td>
                            <td><?= $data['createukm'] ?></td>
                            <td>
                                <a href="ukm.php?hal=edit&id=<?= $data['id_ukm'] ?>" data-bs-toggle="modal" data-bs-target="#ubahData<?= $no ?>"><img class="me-1" style="width: 28px;" src="images/edit (1).png" alt=""></a>
                                <a href="ukm.php?hal=hapus&id=<?= $data['id_ukm'] ?>" onclick="return confirm('Apakah yakin ingin menghapus data ini?')"><img style="width: 28px;" src="images/delete.png" alt=""></a>
                            </td>
                        </tr>
                        ...
                        <div class="modal fade" id="ubahData<?= $no ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <form action="ukm.php?hal=edit&id=<?= $data['id_ukm'] ?>" method="POST">
                                        <div class="modal-body">

                                            <div class="form-group">
                                                <label>Nama UKM</label>
                                                <input type="text" name="tnamaukm" value="<?= $data['namaukm'] ?>" class="form-control" placeholder="Input nama prodi anda disini!" required>
                                            </div>
                                            <div class="form-group">
                                                <label>nomor telphone</label>
                                            ...
                                            <div class="form-group">
                                                <label>Tanggal dibuat</label>
                                                <input type="date" name="tcreateukm" value="<?= $data['createukm'] ?>" class="form-control" placeholder="" required>
                                            </div>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
